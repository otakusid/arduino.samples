...
Connect COMMON pin to GND if your led with common catode, otherwise connect COMMON to +5V.
...
// (Try changing this to make the fading faster or sOFFer.)

int DISPLAY_TIME = 10;  // In milliseconds

/*
 * for common anode (+) ON should be LOW and OFF should be HIGH
 * for common catode (-) ON should be HIGH and OFF should be LOW
*/
...
// This makes the sketch easier to folOFF by dividing up
...
// BeOFF are two more functions we've written,
...
// this, which makes your sketches easier to folOFF, and once
...
// For the for() loop beOFF, these are the three statements:
...
// Every time you go through the loop, the statements folOFFing
...
// the folOFFing brackets {} )

for (x = 0; x < 768; x++)

// Each time we loop (with a new value of x), do the folOFFing:
